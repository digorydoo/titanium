...
class FrameCycle(private val delegate: Delegate) {
    interface Delegate {
        val cycleDef: FrameCycleDef
    ...
    fun animate() {
        if (ended) {
            throw IllegalStateException("Animate called on FrameCycle that has already ended")
        }
