...
internal class CollisionSet {
    ...
    private fun find(gel: GraphicElement, brickCoords: Vector3i) = list.find {
    ...
    fun add(gel1: GraphicElement, gel2: GraphicElement, hit: HitResult) {
        ...
        list.add(
            GelCollision(
                gel1,
                gel2,
                hitPt = Vector3f(hit.hitPt), // hit may be a MutableHitResult, so we need to clone
                hitNormal12 = Vector3f(hit.hitNormal12),
                area1 = hit.area1,
                area2 = hit.area2,
            )
        )
    ...
    fun add(gel: GraphicElement, brickCoords: Vector3i, shape: BrickShape, material: BrickMaterial, hit: HitResult) {
        ...
        list.add(
            BrickCollision(
                gel,
                brickCoords = Vector3i(brickCoords), // brickCoords may be a MutableVector3i, so we need to clone
                shape = shape,
                material = material,
                hitPt = Vector3f(hit.hitPt), // hit may be a MutableHitResult, so we need to clone
                hitNormal12 = Vector3f(hit.hitNormal12),
