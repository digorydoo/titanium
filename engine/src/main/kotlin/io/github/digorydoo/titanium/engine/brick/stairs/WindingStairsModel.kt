...
class WindingStairsModel(
    ...
    private class Step {
        val lowerInner = MutableVector3f()
        val lowerOuter1 = MutableVector3f()
        val lowerOuter2 = MutableVector3f() // step's downside below next step
        val upperInner = MutableVector3f()
        val upperOuter1 = MutableVector3f() // above lowerOuter1
        val upperOuter2 = MutableVector3f() // below next step
        val upFaceTexOuter1 = MutableVector2f()
        val upFaceTexOuter2 = MutableVector2f()
    ...
    private val upFaceTexInner = MutableVector2f()
    ...
    override fun prepare(ix: Int, iy: Int, iz: Int, subvolume: BrickSubvolume) {
        ...
        val texAtAxis = when (lowEdge) {
            Side.SOUTH -> if (ccw) Vector2f.point01 else Vector2f.point11
            Side.EAST -> if (ccw) Vector2f.point11 else Vector2f.point10
            Side.NORTH -> if (ccw) Vector2f.point10 else Vector2f.zero
            Side.WEST -> if (ccw) Vector2f.zero else Vector2f.point01
        }
