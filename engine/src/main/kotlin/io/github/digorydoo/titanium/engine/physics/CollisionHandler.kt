...
import io.github.digorydoo.titanium.engine.brick.BrickFaceCoveringRetriever
...
import io.github.digorydoo.titanium.engine.physics.CollisionManager.Companion.COLLISION_VICINITY
...
import io.github.digorydoo.titanium.engine.physics.strategy.CollisionStrategiesWrapper
...
internal class CollisionHandler {
    ...
    private val hitNormal21 = MutableVector3f()
    private val brick = Brick()
    private val brickCoords = MutableVector3i()
    private val tmpPos1 = MutableVector3f()
    private val tmpPos2 = MutableVector3f()
            ...
            if (strategies.check(body1, body1.nextPos, body2, body2.nextPos, hit)) {
                ...
                if (!BuildConfig.isProduction) {
                    ...
                    if (strategies.check(body1, body1.nextPos, body2, body2.nextPos)) {
                        throw SeparationFailed(body1, Vector3f(tmpPos1), body2, Vector3f(tmpPos2))
                    }
                }
        ...
        if (!gel.shouldBounceOnCollision()) {
            ...
            if (strategies.check(gelBody, gelBody.nextPos, brickBody, brickBody.nextPos, bfcr, brickCoords, hit)) {
                ...
                if (!BuildConfig.isProduction) {
                    ...
                    if (stillCollide) {
                        throw SeparationFailed(gelBody, Vector3f(tmpPos1), brickBody, Vector3f(tmpPos2))
                    }
                }
    ...
    private fun getRigidBody(brickCoords: Vector3i, shape: BrickShape, material: BrickMaterial): RigidBody? {
    ...
    private fun abortIfPushedTooFar(
        body1: RigidBody,
        nextPos1Before: Vector3f,
        body2: RigidBody,
        nextPos2Before: Vector3f,
    ) {
