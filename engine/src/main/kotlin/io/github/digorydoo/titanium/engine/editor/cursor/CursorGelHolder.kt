package io.github.digorydoo.titanium.engine.editor.cursor
...
import io.github.digorydoo.titanium.engine.editor.cursor.CursorGel.Kind.*
...
internal class CursorGelHolder(private val state: EditorState) {
    ...
    fun brickSelectionChanged(box: Boxi) {
        ...
        val lowerNW = MutableVector3f().also { brickToWorld(x0, y0, z0, it) }
        val lowerNE = MutableVector3f().also { brickToWorld(x0, y1, z0, it) }
        val lowerSW = MutableVector3f().also { brickToWorld(x1, y0, z0, it) }
        val lowerSE = MutableVector3f().also { brickToWorld(x1, y1, z0, it) }

        val upperNW = MutableVector3f().also { brickToWorld(x0, y0, z1, it) }
        val upperNE = MutableVector3f().also { brickToWorld(x0, y1, z1, it) }
        val upperSW = MutableVector3f().also { brickToWorld(x1, y0, z1, it) }
        val upperSE = MutableVector3f().also { brickToWorld(x1, y1, z1, it) }
    ...
    fun heightMapSelectionChanged(r: Recti) {
        ...
        val upperNW = MutableVector3f().also {
        ...
        val lowerNW = MutableVector3f(upperNW).also { it.z -= 2 * SMALL_CURSOR_DELTA }
        val lowerNE = MutableVector3f(upperNE).also { it.z -= 2 * SMALL_CURSOR_DELTA }
        val lowerSW = MutableVector3f(upperSW).also { it.z -= 2 * SMALL_CURSOR_DELTA }
        val lowerSE = MutableVector3f(upperSE).also { it.z -= 2 * SMALL_CURSOR_DELTA }
