...
class Tesselator(
    ...
    // Using Pair here instead of Vector2i hoping that it gets inlined
    ...
    fun addFan(
        centre: Vector3f,
        circum: Array<MutableVector3f>,
        texMid: Vector2f,
        texCirc: Array<MutableVector2f>,
        faceIdx: Int,
        ccw: Boolean = true,
        flipTexX: Boolean = false,
    ) {
    ...
    fun addStrip(
        upperPts: Array<MutableVector3f>,
        lowerPts: Array<MutableVector3f>,
        faceIdx: Int,
        close: Boolean,
        texWrapCount: Int,
    ) {
    ...
    private fun computeNormal(a: Vector3f, b: Vector3f, c: Vector3f) =
        (b.toMutable() - a).cross(c - a).normalize()

    private fun addPos(pos: Vector3f) {
        posList.add(pos.x + translation.x)
        posList.add(pos.y + translation.y)
        posList.add(pos.z + translation.z)
    }

    private fun addNormal(normal: Vector3f) {
