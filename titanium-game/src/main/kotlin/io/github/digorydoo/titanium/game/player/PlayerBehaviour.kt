...
import io.github.digorydoo.titanium.game.core.GameSampleId
...
class PlayerBehaviour(
    ...
    private val leftJoyWithCameraCorrection = MutableVector2f()
    ...
    fun animate() {
        if (didCollideWithFloor) {
            if (!hasGroundContact && touchDownSpeed < TOUCHDOWN_MIN_SPEED) {
                ...
                App.sound.play(GameSampleId.TOUCHDOWN, gel.pos, volume)
            }
        ...
        if (sendBehaviour.active) {
            // The player is being sent along a path.
            val speed = gel.body.speed
            val cycleSpeedFactor = clamp(speed.length() / WALK_SPEED, 0.42f, 1.8f)
            orientation = atan2(speed.y, speed.x)
            val dir = getDirFromOrientation()
            frameCycles.walk(dir, cycleSpeedFactor)
            return
        }

        if (App.dlg.isInDlgMode || App.editor.isShown || App.intermissions.anyRunning) {
            return
        }
    ...
    private fun handleInputControls() {
