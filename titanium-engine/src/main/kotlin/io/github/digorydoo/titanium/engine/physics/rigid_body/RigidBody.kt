...
sealed class RigidBody protected constructor(
    ...
    val pos = MutableVector3f(initialPos)
    val speed = MutableVector3f()

    abstract val enclosingRadius: Float // must cover the entire body plus EPSILON

    private val _resultingForce = MutableVector3f()
    val resultingForce: Vector3f get() = _resultingForce

    val nextPos = MutableVector3f()
    val nextSpeed = MutableVector3f()

    private val _speedBeforeCollisions = MutableVector3f()
    val speedBeforeCollisions: Vector3f get() = _speedBeforeCollisions

    private val _normDirOfSpeedBeforeCollisions = MutableVector3f()
    private var isSpeedBeforeCollisionsSignificant = false

    val normDirOfSpeedBeforeCollisions: Vector3f?
