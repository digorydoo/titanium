...
internal class CuboidCheckHelper(private val delegate: Delegate) {
    ...
    class CuboidCheckResults {
        ...
        val hitPt = MutableVector3f(Float.NaN, Float.NaN, Float.NaN)
        val hitNormal12 = MutableVector3f(Float.NaN, Float.NaN, Float.NaN)
        var distanceToClosestPtOnPlane = 0.0f

        fun setDefaults(cuboidFaceNormal: Vector3f) {
    ...
    sealed class CuboidFace(val area: HitArea, val normal: Vector3f) {
        val checkResults = CuboidCheckResults()
    }

    class CuboidVerticalFace(area: HitArea, normal: Vector3f): CuboidFace(area, normal)
    class CuboidHorizontalFace(area: HitArea, normal: Vector3f): CuboidFace(area, normal)
