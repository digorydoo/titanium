...
class UISwishFadeRendererImpl(private val delegate: Delegate): UISwishFadeRenderer() {
    private val shader = Shader(ProgramType.UI_SWISH_FADE)
    ...
    override fun renderTransparent() {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST) // GL_LINEAR = anti-aliasing
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST)

        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE)

        shader.uniforms.setProgress(delegate.progress)
    ...
    companion object {
        private val TAG = Log.Tag("UISwishFadeRendererImpl")
    }
}
