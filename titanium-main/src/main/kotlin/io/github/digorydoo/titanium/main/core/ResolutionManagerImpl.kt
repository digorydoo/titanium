...
import io.github.digorydoo.titanium.engine.core.MILLIMETRES_PER_INCH
import io.github.digorydoo.titanium.engine.core.ResolutionManager
...
class ResolutionManagerImpl: ResolutionManager() {
    ...
    override val screenSizeDp: Vector2i get() = _screenSizeDp
    private val _screenSizeDp = MutableVector2i()

    override val dpToGlFactor: Vector2f get() = _dpToGlFactor
    private val _dpToGlFactor = MutableVector2f()
    ...
    private fun setFullscreenAndUpdatePrefs(monitor: Long, resolution: Resolution) {
        ...
        prefs.nameOfMonitor = glfwGetMonitorName(monitor) ?: ""
        prefs.fullscreenResX = resolution.numPixelsX
        prefs.fullscreenResY = resolution.numPixelsY

        Main.onEnterFullscreen()
    }
    ...
    fun onFramebufferSize(widthArg: Int, heightArg: Int) {
        ...
        if (newWidth <= 0 || newHeight <= 0) {
            // This happens on Windows when the window is minimised.
    ...
    private fun updateViewport() {
        ...
        glfwGetFramebufferSize(Main.window, tmp1, tmp2)
    ...
    override fun setAutoPickMonitorAndResAndUpdatePrefs(auto: Boolean) {
        ...
        if (auto) {
            ...
            setFullscreenMode(prefs.fullscreen)
        }
    }
