...
class Main(private val app: AppImpl) {
    ...
    private fun initialize() {
        ...
        val pathToLogFile = assets.pathToLogFile

        Log.strategies = mutableListOf(
            TtyLogStrategy(
                minSeverity = Severity.INFO,
                output = TtyOutput.EVERYTHING_STDOUT, // stdout, because IntelliJ TTY colours interfere on stderr
                colours = true,
            ),
            FileLogStrategy(
                file = File(pathToLogFile),
        ...
        Log.info(TAG, "Logging to $pathToLogFile")
