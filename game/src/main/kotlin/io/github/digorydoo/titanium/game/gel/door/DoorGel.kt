    ...
    private val actionDelegate = object: ActionDelegate {
        override fun onSelect(action: ActionManager.Action) {
            ...
            App.intermissions.begin {
                showDlg<Unit> {
                    text = "This is a test of intermission. Sleeping for 3 seconds..."
                }
                ...
                showDlg<Unit> {
                    text = "Now let's add a cancellable section."
                }

                cancellable {
                    showDlg<Unit> {
                        text = "You should now see the cutscene strips. Sleeping for 3 seconds..."
                    }

                    sleep(3.0f)

                    showDlg<Unit> {
                        text = "You could have cancelled the part, but you didn't."
                    }
                }

                showDlg<Unit> {
                    text = "Outside cancellable. You should still see the strips. We'll keep the strips until the " +
                        "end. However, you can no longer 'skip' the cutscene. Sleeping..."
                }
                ...
                showDlg<Unit> {
                    text = "You responded with $result"
                }
            }
        }
    }
